...
void* HandleClient(void* /*, struct sockaddr *addr, size_t addrlen,
                  int sock_family*/);
...
void* 
HandleClient(void* arg/*, struct sockaddr *addr, size_t addrlen,
                  int sock_family*/) 
{
  // Casting arg to int*, then assign to file descriptor after dereferencing
  int* int_ptr = (int*)arg;
  int c_fd = *int_ptr;

  // Opening the database file
  int db_fd = open("db.txt", O_RDWR | O_CREAT, S_IRWXU);    


  // Loop, reading data and sending a message back, until the client
  // closes the connection.
  while (1) {
    struct msg *m = (struct msg*) malloc (sizeof(struct msg));
    ssize_t res = read(c_fd, m, sizeof(struct msg));
    
    // Check for errors and if client disconnected, then close the socket if an error occurred
    if (res == 0) {
    ...
    int end = lseek(db_fd, 0, SEEK_END);    //mark the end of the file
    int index = 0;
    
    if (m->type == 1) {
      //PUT
      
      int testint = write(db_fd, &(m->rd), sizeof(struct record));


      if ( sizeof(struct record) == testint) {
    ...
    if (m->type == 2) {
      //GET
      m->type = 5;    //assume search failed until proven otherwise
      struct record temp_rec;
      while (end > lseek(db_fd, index * sizeof(struct record), SEEK_SET)) {
        read(db_fd, &temp_rec, sizeof(struct record));
        if (temp_rec.id == m->rd.id) {
          m->type = 4;
          m->rd = temp_rec;
          break;
        }
  ...
  // Close once the client disconnects
  close(c_fd);
  pthread_exit(NULL);
  return NULL;
}
